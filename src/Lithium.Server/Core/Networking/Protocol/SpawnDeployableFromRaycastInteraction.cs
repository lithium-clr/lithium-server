...
public class SpawnDeployableFromRaycastInteraction : SimpleInteraction
{
    public override int TypeId => 42;
    
    [JsonPropertyName("deployableConfig")] public DeployableConfig? DeployableConfig { get; set; }
    [JsonPropertyName("maxDistance")] public float MaxDistance { get; set; }
    ...
    public override void Serialize(PacketWriter writer)
    {
        ...
        if (DeployableConfig is not null) bits.SetBit(32);
        if (Costs is not null) bits.SetBit(64);
        ...
        var deployableConfigOffsetSlot = writer.ReserveOffset();
        ...
        if (DeployableConfig is not null)
        {
            writer.WriteOffsetAt(deployableConfigOffsetSlot, writer.Position - varBlockStart);
            DeployableConfig.Serialize(writer);
        }
        else writer.WriteOffsetAt(deployableConfigOffsetSlot, -1);
    ...
    public override void Deserialize(PacketReader reader)
    {
        ...
        if (bits.IsSet(32)) DeployableConfig = reader.ReadObjectAt<DeployableConfig>(offsets[5]);
        if (bits.IsSet(64)) Costs = reader.ReadDictionaryAt(offsets[6], r => r.ReadInt32(), r => r.ReadFloat32());
    }

    public override int ComputeSize() => 51;
}