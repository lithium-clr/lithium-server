...
public class SimpleInteraction : Interaction
{
    ...
    public override void Serialize(PacketWriter writer)
    {
        ...
        if (Settings is not null)
        {
            ...
            foreach (var (key, value) in Settings) { writer.WriteEnum(key); value.Serialize(writer); }
        }
    ...
    public override void Deserialize(PacketReader reader)
    {
        ...
        if (bits.IsSet(2)) Settings = reader.ReadDictionaryAt(offsets[1], r => r.ReadEnum<GameMode>(), r => { var s = new InteractionSettings(); s.Deserialize(r); return s; });
