...
public class ParallelInteraction : Interaction
{
    ...
    public override void Serialize(PacketWriter writer)
    {
        ...
        if (Next is not null) bits.SetBit(32);
        ...
        var nextOffsetSlot = writer.ReserveOffset();
        ...
        if (Next is not null)
        {
            writer.WriteOffsetAt(nextOffsetSlot, writer.Position - varBlockStart);
            writer.WriteVarInt(Next.Length);
            foreach (var item in Next) writer.WriteInt32(item);
        }
        else writer.WriteOffsetAt(nextOffsetSlot, -1);
    }
    ...
    public override void Deserialize(PacketReader reader)
    {
        ...
        if (bits.IsSet(32)) Next = reader.ReadArrayAt(offsets[5], r => r.ReadInt32());
    }
