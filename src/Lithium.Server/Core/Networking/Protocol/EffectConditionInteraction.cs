...
public class EffectConditionInteraction : SimpleInteraction
{
    public override int TypeId => 25;
    
    [JsonPropertyName("entityEffects")] public int[]? EntityEffects { get; set; }
    [JsonPropertyName("match")] public Match Match { get; set; } = Match.All;
    ...
    public override void Serialize(PacketWriter writer)
    {
        ...
        if (EntityEffects is not null) bits.SetBit(32);
        ...
        var entityEffectsOffsetSlot = writer.ReserveOffset();
        ...
        {
            writer.WriteOffsetAt(entityEffectsOffsetSlot, writer.Position - varBlockStart);
            writer.WriteVarInt(EntityEffects.Length);
            foreach (var item in EntityEffects) writer.WriteInt32(item);
        }
        else writer.WriteOffsetAt(entityEffectsOffsetSlot, -1);
    }
    ...
    public override void Deserialize(PacketReader reader)
    {
        ...
        if (bits.IsSet(32)) EntityEffects = reader.ReadArrayAt(offsets[5], r => r.ReadInt32());
    }

    public override int ComputeSize() => 45;
}