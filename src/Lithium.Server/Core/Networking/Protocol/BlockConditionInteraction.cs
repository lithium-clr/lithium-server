...
public class BlockConditionInteraction : SimpleBlockInteraction
{
    public override int TypeId => 14;
    
    [JsonPropertyName("matchers")] public BlockMatcher[]? Matchers { get; set; }
    ...
    public override void Serialize(PacketWriter writer)
    {
        ...
        if (Matchers is not null) bits.SetBit(32);
        ...
        var matchersOffsetSlot = writer.ReserveOffset();
        ...
        {
            writer.WriteOffsetAt(matchersOffsetSlot, writer.Position - varBlockStart);
            writer.WriteVarInt(Matchers.Length);
            foreach (var m in Matchers) m.Serialize(writer);
        }
        else writer.WriteOffsetAt(matchersOffsetSlot, -1);
    }
    ...
    public override void Deserialize(PacketReader reader)
    {
        ...
        if (bits.IsSet(32)) Matchers = reader.ReadArrayAt(offsets[5], r => r.ReadObject<BlockMatcher>());
    }
