...
public sealed class ParticleAttractor : INetworkSerializable
{
    ...
    public void Serialize(PacketWriter writer)
    {
        ...
        if (Position is not null) Position.Serialize(writer); else writer.WriteZero(12);
        if (RadialAxis is not null) RadialAxis.Serialize(writer); else writer.WriteZero(12);
        ...
        if (LinearAcceleration is not null) LinearAcceleration.Serialize(writer); else writer.WriteZero(12);
        writer.WriteFloat32(RadialImpulse);
        writer.WriteFloat32(RadialTangentImpulse);
        if (LinearImpulse is not null) LinearImpulse.Serialize(writer); else writer.WriteZero(12);
        if (DampingMultiplier is not null) DampingMultiplier.Serialize(writer); else writer.WriteZero(12);
    }
    ...
    public void Deserialize(PacketReader reader)
    {
        ...
        if (bits.IsSet(1)) Position = reader.ReadObject<Vector3Float>(); else for(int i=0; i<12; i++) reader.ReadUInt8();
        if (bits.IsSet(2)) RadialAxis = reader.ReadObject<Vector3Float>(); else for(int i=0; i<12; i++) reader.ReadUInt8();
        ...
        if (bits.IsSet(4)) LinearAcceleration = reader.ReadObject<Vector3Float>(); else for(int i=0; i<12; i++) reader.ReadUInt8();
        RadialImpulse = reader.ReadFloat32();
        RadialTangentImpulse = reader.ReadFloat32();
        if (bits.IsSet(8)) LinearImpulse = reader.ReadObject<Vector3Float>(); else for(int i=0; i<12; i++) reader.ReadUInt8();
        if (bits.IsSet(16)) DampingMultiplier = reader.ReadObject<Vector3Float>(); else for(int i=0; i<12; i++) reader.ReadUInt8();
    }
}
