...
public class SerialInteraction : Interaction
{
    ...
    [JsonPropertyName("serialInteractions")]
    public int[]? SerialInteractions { get; set; }
    ...
    public override void Serialize(PacketWriter writer)
    {
        ...
        if (SerialInteractions is not null) bits.SetBit(32);
        ...
        var serialInteractionsOffsetSlot = writer.ReserveOffset();
        ...
        if (SerialInteractions is not null)
        {
            writer.WriteOffsetAt(serialInteractionsOffsetSlot, writer.Position - varBlockStart);
            writer.WriteVarInt(SerialInteractions.Length);
            foreach (var item in SerialInteractions) writer.WriteInt32(item);
        }
        else writer.WriteOffsetAt(serialInteractionsOffsetSlot, -1);
    }
    ...
    public override void Deserialize(PacketReader reader)
    {
        ...
        if (bits.IsSet(32)) SerialInteractions = reader.ReadArrayAt(offsets[5], r => r.ReadInt32());
    }

    public override int ComputeSize() => 35;
}